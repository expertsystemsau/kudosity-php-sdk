...
source: xsys-handoff
mode: full
generated_at: "2026-08-06T12:21+10:00"
title: "Kudosity 2.0 — Phase 5 (Laravel) 4 of 6 tasks done on this branch"
status: in-progress
branch: "feat/kudosity-laravel-v2"
head_sha: "e11fb8643dc0acf4fb76246c2a8cfb20bba678e6"
dirty_files: 0
diff_digest: "clean"
ticket_key: "none"
repo: "transmitsms-php-sdk"
submodules: []
next_step: "Phase 5 Task 5 — the three kudosity:webhook:* Artisan commands (list, install, delete), per docs/superpowers/plans/2026-08-06-kudosity-phase-5-laravel.md"
---
...
**This branch is Phase 5 (Laravel integration), four of six tasks in.** Phases 1–4 are merged and pushed on `main` (`a512f02`).
...
- [ ] **Phase 5 Task 5 — the three `kudosity:webhook:*` Artisan commands.** Not started. `list`, `install`, `delete`. **`install` must build its receiver URL through `CallbackUrlBuilder`** — a bare URL will be refused by the receiver's own 403, so an unsigned install produces a webhook that can never deliver. It should also surface Phase 4's HTTPS guard as a readable error rather than a stack trace, and `list` should say "none registered" plainly because the API answers `{}`.
...
| Command | Result |
|---|---|
| `vendor/bin/pest --compact` | 805 passed (1566 assertions) |
...
| CI at `e11fb86` | `run-tests`, `PHPStan`, `Fix PHP code style issues` all SUCCESS |
| Live receiver verification | **not run — this is the outstanding Task 6 step** |
| `composer test-coverage` | not run this session |

Mutation testing ran on every Phase 5 task — 4 on the provider, 7 on the routing/contract, 7 on the channels, 5 on the receiver. All caught except three that were informative rather than gaps (see Failed Approaches).
...
1. Confirm the baseline: `git checkout feat/kudosity-laravel-v2 && vendor/bin/pest --compact && vendor/bin/phpstan analyse --no-progress`
   - Expected: `805 passed (1566 assertions)` and `[OK] No errors`.
   - If different: `git log --oneline -8` — something moved since this handoff.
2. Do **Task 5**: the three `kudosity:webhook:*` commands, per the plan. Build `install`'s URL through `CallbackUrlBuilder`.
   - Expected: an installed webhook whose URL the receiver accepts. If the receiver 403s its own installed URL, the command built a bare URL — that is the bug this warns about.
3. Do the rest of **Task 6**: `packages/kudosity-laravel/README.md`, root `README.md`, `CLAUDE.md`. `UPGRADING.md` and `CHANGELOG.md` are done.
4. Run the **live receiver verification** — the step that proves the phase. Rig recipe in the plan's Task 6 Step 5. Leave the account at zero webhooks.
5. Re-run the full verification block from the plan, then merge to `main` and push. CI on `main` will show `Split Monorepo` failing; that is expected.
